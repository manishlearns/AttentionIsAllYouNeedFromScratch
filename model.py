...
class MultiHeadAttentionBlock(nn.Module):
    ...
    @staticmethod
    def attention(self,query,value,key,mask,dropout: nn.Dropout):
    ...
    def forward(self,q,v,k,mask):
        ...
        x,self.attention_scores = MultiHeadAttentionBlock.attention(self,query,value,key,mask,self.dropout)
...
class EncoderBlock(nn.Module):
    def __init__(self,self_attention: MultiHeadAttentionBlock, feed_forward: FeedForwardNetwork, dropout: float):
        super().__init__()
    ...
    def forward(self,x,src_mask):
        ...
        x = self.residual_connections[1](x,sublayer = self.feed_forward)
...
class DecoderBlock(nn.Module):
    def __init__(self, self_attention_block: MultiHeadAttentionBlock, cross_attention_block: MultiHeadAttentionBlock, feed_forward: FeedForwardNetwork, dropout: float):
        super().__init__()
    ...
    def forward(self,x,encoder_output,src_mask,target_mask): # encoder mask(src_mask is used to avoid padding visibility in encoder)
        ...
        x = self.residual_connections[2](x,sublayer = self.feed_forward)
        return x
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int = 512, h: int = 8, N: int = 6, d_ff: int = 2048, dropout: float = 0.1 ) -> Transformer:
    ...
    for _ in range(N):
        ...
        decoder_self_attention = MultiHeadAttentionBlock(d_model, h, dropout=dropout)
        decoder_cross_attention = MultiHeadAttentionBlock(d_model, h ,dropout=dropout)
        feed_forward = FeedForwardNetwork(d_model,d_ff, dropout=dropout)
